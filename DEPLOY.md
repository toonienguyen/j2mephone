...
2. Ở màn hình **Set up builds and deployments**, điền:

| Trường | Giá trị |
|---|---|
| **Project name** | tuỳ ý, ví dụ `phoneme-ios` (nhớ tên này để dùng ở Bước 5) |
| **Production branch** | `main` |
| **Framework preset** | `None` (không chọn Vite hay framework nào, vì Actions sẽ build thay) |
| **Build command** | để trống, hoặc gõ `echo skip` — không quan trọng |
| **Build output directory** | `web/dist` |
| **Root directory (Advanced)** | để trống (mặc định gốc repo) |

3. Phần **Environment variables** — không cần điền gì, vì các secret chạy
   trong GitHub Actions, không phải trong Cloudflare Git integration
4. Bấm **Save and Deploy** — lần build đầu tiên qua Git integration của
   Cloudflare **sẽ lỗi** (vì thiếu Emscripten), đây là điều bình thường và
   không ảnh hưởng gì — mục đích duy nhất của bước này là để project được
   tạo ra trên tài khoản bạn
5. Sau khi tạo xong, vào **Settings → Build → Branch control**, tắt
